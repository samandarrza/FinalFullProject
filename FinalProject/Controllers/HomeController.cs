    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Index()
        {
            HomeVM homeVM = new HomeVM
            {
                ...
                MostSoldPhones = _context.Phones.Include(x => x.PhoneImages).Include(x => x.Reviews).ThenInclude(x => x.AppUser)
                .Include(x => x.PhoneModel).Include(x => x.Memory).Where(x => x.MostPopular).Take(6).ToList(),
                DiscountedPhones = _context.Phones.Include(x => x.PhoneImages).Include(x => x.Reviews).ThenInclude(x => x.AppUser)
